...
import { PackageSkeleton } from '../BookingSkeletons';
      ...
      <div className='w-full'>
        <div className='mb-16 mt-4 md:mt-8 flex flex-col md:flex-row md:justify-between md:items-end gap-8'>
          <div className='max-w-2xl'>
            ...
            <h2 className='text-4xl md:text-6xl font-heading text-white font-normal mb-0 tracking-tight'>
              ...
              One package per selected service
            </h2>
          </div>

          <div className='max-w-md text-secondary font-body text-base xl:text-lg leading-relaxed mb-1 md:mb-2'>
            Each selected category needs its own package. This keeps pricing and
            delivery expectations clear when services are bundled together.
          </div>
        </div>
        ...
        <div className='space-y-24'>
          {selections.map((selection) => {
            ...
            const title =
              category?.name ?? categoryMeta?.name ?? 'Selected Service';
            const description =
              category?.description ?? categoryMeta?.description;

            return (
              <section key={selection.categoryId} className='space-y-8'>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8'>
                  {isLoading || !category
                    ? [...Array(2)].map((_, i) => <PackageSkeleton key={i} />)
                    : category.packages.map((pkg, idx) => {
                        ...
                        return (
                          <button
                            ...
                            type='button'
                            onClick={() =>
                              selectPackage(selection.categoryId, pkg.id)
                            }
                            className={`
                              group relative text-left overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] outline-none cursor-pointer
                            ...
                            <div className='relative z-30 p-8 sm:p-12 h-full flex flex-col justify-between'>
                              <div className='flex flex-col sm:flex-row justify-between sm:items-start gap-8 sm:gap-4 mb-20'>
                                <div className='flex flex-col order-2 sm:order-1'>
                                  ...
                                  <h4 className='text-2xl md:text-[2rem] font-heading text-white font-medium pr-4'>
                                    {pkg.name}
                                  </h4>
                                </div>
                              ...
                              <div className='border-t border-white/5 pt-8 mt-auto group-hover:border-primary/20 transition-colors duration-700'>
                                {pkg.description && (
                                  <span className='block text-copy/90 text-base font-body mb-8 font-medium leading-[1.8] opacity-90 max-w-lg'>
                                    {pkg.description}
                                  </span>
                                )}
                                ...
                                <ul className='flex flex-col gap-3.5'>
                                  {pkg.includes.map((item, i) => (
                                    ...
                                    >
                                      ...
                                      />
                                      <span
                                        className={`leading-[1.7] transition-colors duration-500 ${isSelected ? 'text-copy/90' : 'group-hover:text-copy/90'}`}
                                      >
                                        {item}
                                      </span>
                                    </li>
